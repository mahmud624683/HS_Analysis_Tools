...
def verilog2dimacs(path):
	...
	key1 = "1\'b1"
	if '1\'b0' in net_map:
		clauses.append(f'-{net_map[key0]} 0')
	if '1\'b1' in net_map:
		clauses.append(f"{net_map[key1]} 0")
...
if __name__ == '__main__':
	...
	top, net_map, clauses = verilog2dimacs(file_path)
	print(f'top: {top}')
	...
	inps = []
	for m in re.findall(r'input (.+?);', data):
		inps += m.replace(' ', '').split(',')
	print(inps)

	nets = set()
	for _, m in re.findall(r'(or|nor|and|nand|not|xor|xnor) \S+\s*\((.+?)\);', data):
	...
	for vals in product(range(2), repeat=len(inps)):
		...
		# Use PowerShell to run the TCL script
		subprocess.run(['powershell.exe', '-Command', 'C:\\IronTcl\\bin\\tclsh.exe test.tcl'])
